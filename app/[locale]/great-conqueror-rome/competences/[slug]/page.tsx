...
export default async function SkillDetailPage({
  ...
  const siblingSkills = getSkillIndex()
    ...
    .slice(0, 6)
    .map((s) => ({
      href: `/great-conqueror-rome/${skillSeg}/${s.slug}`,
  ...
  return (
    <>
      ...
      <div className="max-w-[1320px] mx-auto px-6 py-3.5 text-xs text-muted">
        ...
        <span className="mx-2 text-border">{t("breadcrumb.separator")}</span>
        <Link href="/great-conqueror-rome/competences" className="text-dim">
          {t("nav.skills")}
        </Link>{" "}
        <span className="mx-2 text-border">{t("breadcrumb.separator")}</span>
      ...
      <div className="max-w-[1320px] mx-auto px-6 pb-20 grid lg:grid-cols-[240px_1fr] gap-7">
        <aside className="bg-panel border border-border rounded-lg p-4 h-fit lg:sticky lg:top-20">
          ...
          <ul className="list-none text-sm">
            <li>
              <Link
                href="/great-conqueror-rome/competences"
                className="block px-2 py-1 text-dim no-underline hover:text-gold2"
              >
        ...
        <main>
          ...
          <RelatedRail
            ...
            items={siblingSkills}
            seeAllHref={`/great-conqueror-rome/${skillSeg}#series-${skill.series}`}
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@type": "Article",
            headline: `${displayName} — Great Conqueror Rome`,
            about: {
              "@type": "VideoGame",
