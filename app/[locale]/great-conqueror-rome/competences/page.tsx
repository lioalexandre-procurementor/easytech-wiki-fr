...
import { getSkillIndex } from "@/lib/gcr";
...
export async function generateMetadata({
  ...
  const titles: Record<string, string> = {
    fr: "Compétences de Great Conqueror: Rome — Catalogue complet (FR)",
    en: "Great Conqueror: Rome Skills — Complete Catalog",
    de: "Great Conqueror: Rome Fähigkeiten — Vollständiger Katalog (DE)",
  };
  const descriptions: Record<string, string> = {
    fr: "Toutes les compétences de GCR organisées comme en jeu : Tactiques de terrain, Commandement, Logistique, Défense, Offensive, plus toutes les compétences spécifiques aux généraux avec progression L1→L5.",
    en: "Every GCR skill organised as in-game: Field Tactics, Command, Logistics, Defense, Offense, plus all general-specific skills with L1→L5 progression.",
    de: "Alle GCR-Fähigkeiten wie im Spiel gegliedert: Feldtaktik, Kommando, Logistik, Verteidigung, Offensive, sowie alle generalspezifischen Fähigkeiten mit Progression L1→L5.",
  };
  return {
  ...
  return (
    <>
      ...
      <div className="max-w-[1320px] mx-auto px-6 py-3.5 text-xs text-muted">
        ...
        <Link href="/great-conqueror-rome" className="text-dim">
          {t("nav.gcr")}
        </Link>{" "}
        <span className="mx-2 text-border">{t("breadcrumb.separator")}</span>
      ...
      <div className="max-w-[1320px] mx-auto px-6 pb-20 grid lg:grid-cols-[240px_1fr] gap-7">
        <aside className="bg-panel border border-border rounded-lg p-4 h-fit lg:sticky lg:top-20">
          ...
          <ul className="list-none text-sm">
            ...
            <li>
              <Link
                href="/great-conqueror-rome/unites-elite"
                className="block px-2 py-1 text-dim no-underline hover:text-gold2"
              >
                🏅 {t("nav.eliteUnits")}
              </Link>
        ...
        <main>
          ...
          <SkillsBrowserClient
            ...
            isFr={isFr}
            isDe={isDe}
            hrefBase="/great-conqueror-rome/competences"
            t={{
