...
import { getUnitsByFaction, getCategoryMeta } from "@/lib/gcr";
    ...
    <>
      ...
      <div className="max-w-[1320px] mx-auto px-6 pb-20 grid lg:grid-cols-[240px_1fr] gap-7">
        <aside className="bg-panel border border-border rounded-lg p-4 h-fit lg:sticky lg:top-20">
          ...
          <ul className="list-none">
            <li><Link href="/great-conqueror-rome" className="block px-2 py-1 rounded text-sm text-dim no-underline hover:text-gold2">{t("nav.backToHubGcr")}</Link></li>
            <li><Link href="/great-conqueror-rome/conquete-romaine" className="block px-2 py-1 rounded text-sm text-dim no-underline hover:text-gold2">🦅 {t("nav.barbarian")}</Link></li>
            <li><Link href="/great-conqueror-rome/generaux" className="block px-2 py-1 rounded text-sm text-dim no-underline hover:text-gold2">👨‍✈️ {t("nav.generals")}</Link></li>
          </ul>
        </aside>
