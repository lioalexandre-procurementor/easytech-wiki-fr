...
import { getAllSlugs, getEliteUnit, getCategoryMeta, COUNTRY_FLAGS, getUnitsByCategory, getFactionMeta, getAllGenerals } from "@/lib/ew6";
import { countryLabel } from "@/lib/countries";
import { localizedUnitField } from "@/lib/localized-copy";
import type { UnitData } from "@/lib/types";

/**
 * Placeholder detection — EW6 entities auto-generated from decrypted game
 ...
 */
function isPlaceholderUnit(u: UnitData | null): boolean {
  if (!u) return false;
...
export async function generateMetadata({ params }: { params: { locale: string; slug: string } }): Promise<Metadata> {
  ...
  const titleByLocale: Record<string, string> = {
    fr: `${u.name} (EW6) — Stats, perks niveau 1-12 & généraux | Wiki FR`,
    en: `${u.name} (EW6) — Stats, level 1-12 perks & generals | Wiki`,
    de: `${u.name} (EW6) — Werte, Stufen 1–12 Boni & Generäle | Wiki DE`,
  };
  const shortDescLocalized = localizedUnitField(u as unknown as Record<string, unknown>, "shortDesc", locale);
  const descByLocale: Record<string, string> = {
    fr: `Fiche complète de ${u.name} dans European War 6 : ${shortDescLocalized} Stats détaillées, perks niveau par niveau, généraux recommandés.`,
    en: `Complete profile of ${u.nameEn || u.name} in European War 6: ${shortDescLocalized} Detailed stats, per-level perks, recommended generals.`,
    de: `Komplettes Profil von ${u.nameEn || u.name} in European War 6: ${shortDescLocalized} Detaillierte Werte, Boni pro Stufe, empfohlene Generäle.`,
  };
  return {
    title: titleByLocale[locale] ?? titleByLocale.en,
    description: descByLocale[locale] ?? descByLocale.en,
    alternates: pageAlternates(locale, {
      fr: `/european-war-6/unites-elite/${params.slug}`,
      en: `/european-war-6/elite-units/${params.slug}`,
      de: `/european-war-6/elite-units/${params.slug}`,
    }),
    ...
    // status still drives `placeholder` below to suppress ad slots, but
  ...
  return (
    <>
      ...
      <div className="max-w-[1320px] mx-auto px-6 py-3.5 text-xs text-muted">
        ...
        <span className="mx-2 text-border">{t("breadcrumb.separator")}</span>
        <Link href="/european-war-6/unites-elite" className="text-dim">{t("nav.eliteUnits")}</Link>{" "}
        <span className="mx-2 text-border">{t("breadcrumb.separator")}</span>
        <span>{unit.name}</span>
      </div>
      ...
      <div className="max-w-[1320px] mx-auto px-3 md:px-6 pb-20 lg:grid lg:grid-cols-[240px_1fr] lg:gap-7">
        <aside className="hidden lg:block bg-panel border border-border rounded-lg p-4 h-fit lg:sticky lg:top-20">
          ...
          <ul className="list-none text-sm">
            {sidebarUnits.map(u => (
              <li key={u.slug}>
                <Link href={`/european-war-6/unites-elite/${u.slug}` as any}
          ...
          <ul className="list-none text-sm">
            <li><Link href="/european-war-6/unites-elite" className="block px-2 py-1 text-dim no-underline hover:text-gold2">{tL("← Toutes les unités d'élite", "← All elite units", "← Alle Elite-Einheiten")}</Link></li>
            {isScorpion && (
              <li><Link href="/european-war-6/empire-du-scorpion" className="block px-2 py-1 text-dim no-underline hover:text-gold2">🦂 {t("nav.scorpion")}</Link></li>
            )}
            <li><Link href="/european-war-6/generaux" className="block px-2 py-1 text-dim no-underline hover:text-gold2">👨‍✈️ {t("nav.generals")}</Link></li>
          </ul>
        </aside>
            ...
            <div className="grid md:grid-cols-2 gap-5">
              <div>
                ...
                {matchedGenerals.length > 0 ? (
                  <RecommendedGeneralsPodium
                    game="ew6"
                    leaderboardHref={`/leaderboards?game=ew6&tab=generals` as never}
                    generalHrefBuilder={(slug) => `/european-war-6/generaux/${slug}`}
                  ...
                  <Link href="/european-war-6/generaux" className="text-dim text-sm no-underline hover:text-gold2">
                ...
                {(() => {
                  const eligible = getEligibleGeneralsForUnit("ew6", unit.slug);
          ...
          {sameCat.length > 0 && (
            <>
              ...
              <div className="grid md:grid-cols-3 gap-4">
                {sameCat.map(u => (
                  <Link key={u.slug} href={`/european-war-6/unites-elite/${u.slug}`}
