...
export async function generateMetadata({
  ...
  const titles: Record<string, string> = {
    fr: "Tous les généraux de European War 6: 1914 (FR) — Tier List & Guides",
    en: "All European War 6: 1914 Generals — Tier List & Guides",
    de: "Alle Generäle von European War 6: 1914 — Tier-Liste & Guides",
  };
  const descriptions: Record<string, string> = {
    fr: "Liste complète des généraux de EW6 : Napoléon, Wellington, Kutuzov, Bismarck. Compétences, bonus et régiments recommandés.",
    en: "Full list of EW6 generals: Napoleon, Wellington, Kutuzov, Bismarck. Skills, bonuses and recommended regiments.",
    de: "Vollständige Liste der EW6-Generäle: Napoleon, Wellington, Kutusow, Bismarck. Fähigkeiten, Boni und empfohlene Regimenter.",
  };
  return {
...
export default async function GCRGeneralsList({
  ...
  // EW6 has no secondary faction split; keep the list empty so the
  // downstream rendering code (copied from GCR) no-ops gracefully.
  const barbarian: typeof all = [];
    ...
    <>
      ...
      <div className="max-w-[1320px] mx-auto px-6 pb-20 grid lg:grid-cols-[240px_1fr] gap-7">
        <aside className="bg-panel border border-border rounded-lg p-4 h-fit lg:sticky lg:top-20">
          ...
          <ul className="list-none text-sm">
            <li><Link href="/european-war-6" className="block px-2 py-1 text-dim no-underline hover:text-gold2">{t("nav.backToHubGcr")}</Link></li>
            <li><Link href="/european-war-6/unites-elite" className="block px-2 py-1 text-dim no-underline hover:text-gold2">🏅 {t("nav.eliteUnits")}</Link></li>
          </ul>
        </aside>
        ...
        <main>
          ...
          >
            ...
            <p className="text-dim max-w-3xl">
              {params.locale === "fr" ? (
                <>
                  {all.length} commandants répertoriés : les meilleurs généraux de l'ère
                  1800–1914 (Napoléon, Wellington, Koutouzov, Bismarck…).
                  Chaque fiche détaille les compétences, attributs et stratégies de combat.
                </>
              ) : params.locale === "de" ? (
                <>
                  {all.length} Kommandanten katalogisiert: die besten Generäle der Epoche
                  1800–1914 (Napoleon, Wellington, Kutusow, Bismarck…).
                  Jeder Eintrag behandelt Fähigkeiten, Attribute und Kampfstrategien.
                </>
              ) : (
                <>
                  {all.length} commanders catalogued: the best generals of the 1800–1914
                  era (Napoleon, Wellington, Kutuzov, Bismarck…).
            ...
            <div className="mt-4 p-3 bg-amber-500/10 border border-amber-500/30 rounded text-amber-200 text-xs">
              ⚠️ <strong>{t("ew6Hub.underConstruction")}</strong> — {t("ew6Hub.underConstructionNote")}
            </div>
          </section>
