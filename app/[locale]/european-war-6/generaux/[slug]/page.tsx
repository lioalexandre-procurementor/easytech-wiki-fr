...
export async function generateMetadata({
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `/${locale}/european-war-6/${locale === "fr" ? "generaux" : "generals"}/${slug}`,
      languages: {
        fr: `/fr/european-war-6/generaux/${slug}`,
        en: `/en/european-war-6/generals/${slug}`,
        de: `/de/european-war-6/generals/${slug}`,
        "x-default": `/fr/european-war-6/generaux/${slug}`,
      },
    },
    openGraph: {
      title,
  ...
  return (
    <>
      ...
      <div className="max-w-[1320px] mx-auto px-6 py-3.5 text-xs text-muted">
        ...
        <span className="mx-2 text-border">{t("breadcrumb.separator")}</span>
        <Link href="/european-war-6/generaux" className="text-dim">{t("nav.generals")}</Link>{" "}
        <span className="mx-2 text-border">{t("breadcrumb.separator")}</span>
        <span>{g.name}</span>
      </div>
        ...
        <main>
          ...
          <div className="mb-4 flex flex-wrap gap-2">
            <Link
              href={
                params.locale === "fr"
                  ? `/european-war-6/generaux/${g.slug}/entraine`
                  : `/european-war-6/generals/${g.slug}/trained`
              }
            ...
            {g.hasTrainingPath && g.trainedSkills && g.trainedSkills.length > 0 && (
              <Link
                href={
                  params.locale === "fr"
                    ? `/european-war-6/generaux/${g.slug}/entrainement-premium`
                    : `/european-war-6/generals/${g.slug}/premium-training`
                }
...
function SkillBlock({
  ...
  const skillDetailHref = skill.skillSlug
    ? `/european-war-6/competences/${skill.skillSlug}`
    : null;
