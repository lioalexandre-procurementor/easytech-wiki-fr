    ...
    <>
      ...
      <div className="max-w-[1320px] mx-auto px-6 pb-20 grid lg:grid-cols-[240px_1fr] gap-7">
        <aside className="bg-panel border border-border rounded-lg p-4 h-fit lg:sticky lg:top-20">
          ...
          <ul className="list-none">
            <li><Link href="/world-conqueror-4" className="block px-2 py-1 rounded text-sm text-dim no-underline hover:text-gold2">{t("nav.backToHubWc4")}</Link></li>
            <li><Link href="/world-conqueror-4/empire-du-scorpion" className="block px-2 py-1 rounded text-sm text-dim no-underline hover:text-gold2">🦂 {t("nav.scorpion")}</Link></li>
            <li><Link href="/world-conqueror-4/generaux" className="block px-2 py-1 rounded text-sm text-dim no-underline hover:text-gold2">👨‍✈️ {t("nav.generals")}</Link></li>
          </ul>
        </aside>
            ...
            return (
              <section key={cat} id={cat} className="mb-8">
                ...
                <div className="grid gap-2.5">
                  {units.map(u => <UnitRow key={u.slug} unit={u} locale={params.locale} game="wc4"/>)}
                </div>
              </section>
            );
          })}
