      ...
      <div className="max-w-[1320px] mx-auto px-3 md:px-6 pb-20 lg:grid lg:grid-cols-[240px_1fr] lg:gap-7">
        <aside className="hidden lg:block bg-panel border border-border rounded-lg p-4 h-fit lg:sticky lg:top-20">
          ...
          <ul className="list-none text-sm">
            ...
            <li><Link href="/world-conqueror-4/generaux" className="block px-2 py-1 text-dim no-underline hover:text-gold2">👨‍✈️ {t("nav.generals")}</Link></li>
          </ul>
        </aside>
              ...
              <div>
                ...
                {matchedGenerals.length > 0 ? (
                  <RecommendedGeneralsPodium
                    game="wc4"
                    enableTrainingBadge
                    leaderboardHref={`/leaderboards?game=wc4&tab=generals` as never}
                    generalHrefBuilder={(slug) => `/world-conqueror-4/generaux/${slug}`}
                    premiumTrainingHrefBuilder={(slug) =>
                      params.locale === "fr"
                    ...
                    generals={matchedGenerals.slice(0, 3).map<PodiumGeneral>((g) => ({
                      ...
                      hasTrainedSkills: !!(g.trainedSkills && g.trainedSkills.length > 0),
                    }))}
                    labels={{
                ...
                {/* (vote component below — wrapping <div> removed in redesign) */}
